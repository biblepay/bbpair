var CURRENT_COIN = 'BBP';
var PARAMS = {
	'BBP': {
		coingecko: 'biblepay',
		coinjs: cc.bitcoin,
		network: cc.bitcoin.networks.biblepay,
		qrColor: '3875CE',
		minFee: 0.1,
		maxFee: 50.0,
		txFee: 0.25,
		explorer: 'https://chainz.cryptoid.info/bbp/',
		donation: '',
		unspentApi: 'http://utxo.biblepay.org:12001/rest/getaddressutxos/',
		unspentSuffix: '',
		sendApi: 'http://utxo.biblepay.org:12001/rest/pushtx/',
		sendTxHex: 'tx_hex',
		sendTxid1: 'txid',
		unspentTxid: 'txid',
        ...
        unspentSuffix: '?unspentOnly=true',
...
$(function() {
    $.ajax({
          url: 'https://chainz.cryptoid.info/api/info',
          cache: false
...
// BBP Air (hash of E-mail address + passphrase hashed 777 times)
...
function login() {
  ...
  { alert("You need a stronger password! Try choosing a longer password(at least 14 characters recommended), with both uppercase and lowercase characters as well as numbers and special characters."); return; }
...
function refresh() {
  $.ajax({
    url: PARAMS[CURRENT_COIN].unspentApi + keyPair.getAddress() + PARAMS[CURRENT_COIN].unspentSuffix,
    type: "GET",
    dataType: "json",
    data: {
    },
...
function copyPrivateKey() {
  var result = prompt("Are you sure you would like to copy your private key to the clipboard?  Click <OK> to continue or <CANCEL>:", "Y");
  if(result == 'Y') {
    try {
...
function spendf() {
  ...
  if(balance < FEE || BBP(amount+FEE) > balance) { alert("Insufficient funds! Minimum network fee is " + FEE + " " + CURRENT_COIN + "."); return; }
  ...
  var change = BBP(balance - amount - FEE);
  if (change > 0)
  {
  ...
  $.ajax({
	...
	if (txid && (typeof txid === 'string' || txid instanceof String)) {
	   ...
	   console.log(result);
	   alert("Broadcast failed! Check debug console for details! NOTE: This wallet must rest for a couple blocks after each send.  This is because spent UTXOs must be confirmed on the chain first.  If you just sent a transaction, please wait until the next block and try again.  ");
	}
